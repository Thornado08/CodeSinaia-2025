...
batch = 1000  # number of events to process
sample_size = 350
count = 0
pos = 0
neg = 0
...
with open("../_Data/output-Set1.txt") as fin:
    ...
    while True:
        ...
        for _ in range(batch):
            ...
            particles = [fin.readline().split() for _ in range(num_particles)]
            event_data.append(particles)

        if len(event_data) == 0:
            break

        sampled_events = random.sample(event_data, min(sample_size, len(event_data)))

        pos_subset = 0
        neg_subset = 0

        for particles in sampled_events:
        ...
        pos_per_sub.append(pos_subset)
        neg_per_sub.append(neg_subset)
        subset_ind.append(batch * count)
        count += 1

avg_pos = pos / total_sampled_events
avg_neg = neg / total_sampled_events

poisson_pos = poisson_distribution(avg_pos)
poisson_neg = poisson_distribution(avg_neg)

diff = difference(pos, neg)
comb_uncertainty = combined_uncertainty(pos, neg)
sig = significance(pos, neg, comb_uncertainty)

print(f"In {total_sampled_events} total events, we had {pos} positive pions and {neg} negative pions.")
...
elapsed_subsample = end_time - start_time
...
try:
    with open("batching_runtime.txt", "r") as f:
        elapsed_batch = float(f.read())
except Exception:
    elapsed_batch = 0.0
...
neg_per_batch = []
try:
    with open("batching_results.txt", "r") as f:
        for line in f:
            ind, pos_b, neg_b = line.strip().split(",")
            batch_ind.append(int(ind))
            pos_per_batch.append(int(pos_b))
            neg_per_batch.append(int(neg_b))
except Exception:
    print("Batching results file not found.")

# Subsampling (cold colours)
plt.plot(subset_ind, pos_per_sub, color='dodgerblue', label="Positive pions (subsampling, 350 events/batch)")
plt.plot(subset_ind, neg_per_sub, color='deepskyblue', linestyle='--', label="Negative pions (subsampling, 350 events/batch)")

# Batching (warm colours)
plt.plot(batch_ind, pos_per_batch, color='orangered', label="Positive pions (batching, 1000 events/batch)")
plt.plot(batch_ind, neg_per_batch, color='orange', linestyle='--', label="Negative pions (batching, 1000 events/batch)")

plt.xlabel("Event Number")
plt.ylabel("Number of Pions in Batch")
plt.title("Positive and Negative Pions: Subsampling vs Batching")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()

methods = ['Subsampling (350 events)', 'Batching (1000 events)']
times = [elapsed_subsample, elapsed_batch]
...
for bar in bars:
    yval = bar.get_height()
    plt.text(bar.get_x() + bar.get_width()/2, yval + 1, f"{yval:.2f}s", ha='center', va='bottom')

plt.title("Runtime Comparison: Subsampling vs Batching")
